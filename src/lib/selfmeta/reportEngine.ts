...
function buildAnamnezFitSection(
  ...
  const priorityDomains = selectNarrativelyMatchedDomains(domainResults, anamnezFlags);
  const priorityText = priorityDomains.map((d) => d.label).join(", ");

  const paragraph =
    anamnezSummary.length > 0
      ? `${anamnezSummary.slice(0, 2).join(" ")} Test bulgularında öncelikli olarak şu alanlar öne çıkmaktadır: ${priorityText}.`
      : `Anamnezde "${profileType}" örüntüsünü destekleyen klinik tema sınırlı bulunmuştur. Test bulgularında öncelikli olarak şu alanlar öne çıkmaktadır: ${priorityText}.`;

  const bullets =
    anamnezFlags.length > 0
      ? anamnezFlags.slice(0, 3).map((f) => `- ${f}`).join("\n")
      : "- Anamnezden çıkarılabilen klinik tema sınırlıdır.";
...
function buildConclusion(
  ...
  const base =
    globalLevel === "Tipik"
      ? "Sonuçlar genel olarak düzenli bir self-regülasyon görünümüne işaret etmekle birlikte klinik gözlemle birlikte ele alınmalıdır."
      : globalLevel === "Riskli"
      ? "Sonuçlar, işlevsel güçlüklerin bağlama göre belirginleşebileceğini ve klinik izlemin yararlı olacağını düşündürmektedir."
      : "Sonuçlar, çoklu alanlarda belirgin self-regülasyon güçlüğü olasılığını düşündürmekte olup ayrıntılı klinik değerlendirme ile birlikte yorumlanmalıdır.";

  const normNote =
    normSource === "age_band_heuristic"
      ? "Bu yorum yaş-duyarlı geçici klinik bantlar kullanılarak üretilmiştir."
      : "Bu yorum sabit sistem içi eşiklerle üretilmiştir.";

  return `${base} Mevcut örüntü "${profileType}" başlığı altında ele alınabilir. ${normNote} Bu rapor tek başına tanısal karar amacıyla kullanılmamalı, anamnez, klinik gözlem ve diğer değerlendirmelerle birlikte yorumlanmalıdır.`;
}
...
export function generateDeterministicReport(input: ReportInput): DeterministicReport {
  ...
  const sections = {
    ...
    conclusion: buildConclusion(globalLevel, profileType, normSource),
  };
