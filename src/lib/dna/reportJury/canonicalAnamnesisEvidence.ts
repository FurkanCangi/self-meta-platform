...
const DOMAIN_PATTERNS: Readonly<Record<DomainKey, RegExp>> = Object.freeze({
  physiological: /(?:\buyku|uykusuz|uyuduğ|uyand|uyuma|\byorgunluğ|biriken\s+yorgunluk|dinlenmiş\s+gün|dinlenmis\s+gun|dinlenme(?:den|nin)?\s+(?:sonra|ardından)|dinlenme\s+sonrası|fizyolojik\s+regülasyon|sabah\s+kalk|nefes(?:i)?\s+hızlan|nefes(?:i)?\s+hizlan|başını\s+masaya|basini\s+masaya)/iu,
  ...
  interoception: /(?:interosepsiyon|açlık|açlığ|aclik|aclig|açken|acken|susuz|susad|ara\s+öğün|ara\s+ogun|ısıya\s+yanıt|isiya\s+yanit|ısınma|isinma|üşüdüğ|usudug|hırka|hirka|tuvalet|ağrı|agri|baş\s+ağr|bas\s+agr|beden\s+sinyal|rahatsızlığ[ıi]n[ıi]\s+sözel|rahatsizligini\s+sozel)/iu,
})
...
const PRESERVED_SUPPLEMENTAL_PATTERN = /(?:tamamlayıp|tamamlayarak|servise\s+yetişiyor|servise\s+yetisiyor|(?:su|mola|tuvalet)\s+istiyor|sırayı\s+bekliyor|sirayi\s+bekliyor|(?:^|[\s,])(?:tamamlar|bitirir|sürdürür|giyer|yerleştirir|yapar)(?=$|[\s.!?,;]))/iu
...
const DIFFICULTY_PATTERN = /(?:zorlan|\bzor\b|güçlük|yapam|başlayam|baslayam|etmiyo|giyem|bitirem|tamamlamıyor|sürdüremedi|surduremedi|devam\s+edemiyor|yalnız\s+(?:ilkini|birini)\s+yanıtlıyor|yalniz\s+(?:ilkini|birini)\s+yanitliyor|fark\s+etmeden|iki\s+eşyadan\s+sonra\s+duruyor|iki\s+esyadan\s+sonra\s+duruyor|\btask\b[^.!?]{0,60}\bortasında\s+duruyor|\btask\b[^.!?]{0,60}\bortasinda\s+duruyor|yarım(?:\s+kal)?|yarıda|yarida|bırak(?![ıi]ld[ıi]ğ[ıi]nda|ildiginda)|\bkaç(?:ıyor|tı|ıp|arken|maya|mak)\b|\bkac(?:iyor|ti|ip|arken|maya|mak)\b|\bkaçın(?:ıyor|dı|maya)\b|\bkacin(?:iyor|di|maya)\b|ayrıl|ayril|uzaklaş|uzaklas|etkinliğ[ıi]\s+durdur|etkinligi\s+durdur|yetişkinin\s+arkasına\s+geç|yetiskinin\s+arkasina\s+gec|göreve\s+dönem|goreve\s+donem|rutinin\s+devamını\s+kesintiye\s+uğrat|rutinin\s+devamini\s+kesintiye\s+ugrat|duygusal\s+yoğunluğ[ıi]\s+art|duygusal\s+yogunlugu\s+art|sakinleşme\s+süresi[^.!?]{0,45}uz|sakinlesme\s+suresi[^.!?]{0,45}uz|hangi\s+adımdan[^.!?]{0,80}bulamad|hangi\s+adimdan[^.!?]{0,80}bulamad|yetişkin\s+yönlendirmesi\s+bekliyor|yetiskin\s+yonlendirmesi\s+bekliyor|başını\s+çek|basini\s+cek|çekil|cekil|kulaklarını kapat|kulak kapat|(?:^|[\s,.;])(?:eğil|egil)|\bağl|\bagla|\bbağır|\bbagir|\bçığlık|\bciglik|unut|\batla|\batl[ıi]|karıştır|karistir|dağıl|dagil|çok\s+yorgun|gecik|geç\s+(?:fark|söyl)|(?:yanlış|yanlis|ters|eksik|hatalı|hatali)[^.!?]{0,45}(?:koy|yerleştir|yerlestir|eşle|esle|seç|sec|yap|uygula)|(?:sonraki\s+)?adıma\s+geçmiyor|(?:sonraki\s+)?adima\s+gecmiyor|sıradan\s+çık|redded|son\s+ana|son anda|son dakka|belirtmeyip|oyunu\s+aniden\s+kes|geç bildir|gec bildir|ancak[^.]{0,60}sonra bildir|demiyo|uzun hatırlatma|hatırlatma gerek|hatirlatma gerek|yere\s+(?:otur|yat|uzan)|uzanıyor|masadan\s+(?:sık\s+)?kalk(?!madan\b)|yavaş tamam|yavas tamam|çıkmak ist|cikmak ist|çıkmaya çalış|cikmaya calis|geri\s+dön(?:müyor|medi|emiyor)|geri\s+don(?:muyor|medi|emiyor)|oyuna\s+dön(?:müyor|medi|emiyor)|oyuna\s+don(?:muyor|medi|emiyor)|masan[ıi]n\s+alt[ıi]na|masa altı)/iu
...
const TRIGGER_PATTERN = /(?:[^,.;]{0,70}(?:olduğunda|oldugunda|[a-zçğıöşü]+(?:dığında|diğinde|duğunda|düğünde|tığında|tiğinde|tuğunda|tüğünde|diginde|tiginda)|[a-zçğıöşü]+(?:ınca|ince|unca|ünce)|sırasında|sirasinda|sonrası|sonrasinda|ardından|ardindan))/iu
...
function normalizedFact(clause: string): string {
  const withoutLabel = clean(clause.replace(
    /^(?:başvuru sebeb[ıi]|basvuru sebeb[ıi]|başvuru|basvuru|başvuru notu|basvuru notu|reason for referral|referral_reason|parent_concerns_goals|parent concerns goals|strengths?|diagnosis|tan[ıi]|ad_soyad|client_code|age)(?:\s*:\s*|\s+)/iu,
    "",
  ))
