...
export function observeStudentCaseContext(message: string): StudentCaseContext {
  ...
  const adultSupport = /\b(?:ogretmen|yetiskin)\w*\b/u.test(normalized)
    && (/\b(?:yavas\w*|yumusat\w*|sakin\w*|yan(?:ina|inda)\w*|destek\w*|bekle\w*)\b/u.test(normalized)
      || /\bgel\w*.{0,40}\b(?:duzel|sakinles|don)\w*\b/u.test(normalized))
  const activityMentioned = /\b(?:oyun|etkinlik)\w*\b/u.test(normalized)
  const environmentalLoad = /\b(?:kalabalik|gurultu)\w*\b/u.test(normalized)
    || /\bsesli\s+(?:ortam|sinif|yer)\w*\b/u.test(normalized)
    || /\bses\s+(?:ortam|sinif|yer)\w*\b/u.test(normalized)
