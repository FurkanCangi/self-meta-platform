...
export default function NewClientPage() {
  ...
  const onCreate = async () => {
    ...
    if (error) {
      setSaving(false);
      setErr("Kayıt oluşturulamadı: " + error.message);
      return;
    }

    setSaving(false);
    const qs = new URLSearchParams({ client: data.child_code }).toString();
        ...
        <div className="mt-1 flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="mt-1 max-w-3xl text-sm leading-6 text-slate-500">
              Tüm anamnez alanları zorunludur. Kayıt oluşturulunca danışan Supabase (public.clients) tablosuna yazılır ve ardından skor girişine geçilir.
            </p>
          </div>
            ...
            {tab === "demo" && (
              <div className="space-y-6">
                <div className="grid gap-4 md:grid-cols-2">
                  ...
                  <Field label="Danışan Kodu" hint="Örn: SM-014 (anonim kod)">
