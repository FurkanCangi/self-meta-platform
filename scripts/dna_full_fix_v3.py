...
targets = [
    Path("src/lib/dna/aiRewrite.ts"),
    Path("src/lib/dna/aiClinicalPrompt.ts"),
    Path("src/lib/dna/clinicalAnalysis.ts"),
...
def patch_ai_rewrite():
    p = Path("src/lib/dna/aiRewrite.ts")
    if not p.exists():
        return
    txt = read(p)
    ...
    if "DNA_REPORT_BUDGET_USD" not in txt:
        anchor = re.search(r'(function\s+getClient\s*\(\)\s*{[\s\S]*?return\s+new\s+OpenAI\([\s\S]*?\)\s*[\r\n]+\})', txt)
        block = '''
const DNA_MODEL = process.env.OPENAI_REPORT_MODEL || "gpt-5"
const DNA_MAX_OUTPUT_TOKENS = Number(process.env.OPENAI_REPORT_MAX_OUTPUT_TOKENS || "2200")
const DNA_TEMPERATURE = Number(process.env.OPENAI_REPORT_TEMPERATURE || "0.35")
const DNA_BUDGET_USD = Number(process.env.DNA_REPORT_BUDGET_USD || "0.1")
'''
        if anchor:
            txt = txt[:anchor.end()] + "\n\n" + block + txt[anchor.end():]
        else:
            txt = block + "\n" + txt
    txt = re.sub(r'model\s*:\s*process\.env\.OPENAI_REPORT_MODEL\s*\|\|\s*["\'][^"\']+["\']', 'model: DNA_MODEL', txt)
    txt = re.sub(r'model\s*:\s*["\'][^"\']*gpt[^"\']*["\']', 'model: DNA_MODEL', txt)
    if "max_output_tokens: DNA_MAX_OUTPUT_TOKENS" not in txt and "max_completion_tokens: DNA_MAX_OUTPUT_TOKENS" not in txt:
        txt = re.sub(r'(\{\s*model\s*:\s*DNA_MODEL\s*,)', r'\1\n      temperature: DNA_TEMPERATURE,\n      max_output_tokens: DNA_MAX_OUTPUT_TOKENS,', txt, count=1)
        txt = re.sub(r'(\{\s*model\s*:\s*DNA_MODEL\s*,)', r'\1\n      temperature: DNA_TEMPERATURE,\n      max_completion_tokens: DNA_MAX_OUTPUT_TOKENS,', txt, count=1)
    if "OPENAI_REPORT_MAX_OUTPUT_TOKENS" not in txt:
        txt += '''
export function getDnaModelConfig() {
  return {
    model: DNA_MODEL,
    temperature: DNA_TEMPERATURE,
    maxOutputTokens: DNA_MAX_OUTPUT_TOKENS,
    budgetUsd: DNA_BUDGET_USD,
  }
}
'''
    write(p, txt)
...
def patch_ai_prompt():
    p = Path("src/lib/dna/aiClinicalPrompt.ts")
    if not p.exists():
        return
    txt = read(p)
    guide = '''
...
`
'''.strip()
    if "DNA_CLINICAL_STYLE_GUIDE" not in txt:
    ...
    for rgx in [
        ...
        m = re.search(rgx, txt)
        if m:
            if "${DNA_CLINICAL_STYLE_GUIDE}" not in txt[m.start():m.start()+300]:
                txt = txt[:m.end()] + "${DNA_CLINICAL_STYLE_GUIDE}\n\n" + txt[m.end():]
            injected = True
            break
    if not injected and "DNA_CLINICAL_STYLE_GUIDE" not in txt[-500:]:
        txt += "\n\nexport const DNA_PROMPT_MIN_WORD_TARGET = 450\n"
    write(p, txt)

def patch_clinical_analysis():
    p = Path("src/lib/dna/clinicalAnalysis.ts")
    if not p.exists():
        return
    txt = read(p)
    helper = '''
function dnaNormalizeText(input: unknown): string {
  return String(input ?? "")
...
function dnaCountHits(source: string, terms: string[]): number {
  const s = dnaNormalizeText(source)
  return terms.reduce((acc, term) => acc + (s.includes(dnaNormalizeText(term)) ? 1 : 0), 0)
}

function deriveDnaConfidence(input: {
  ...
  for (const domain of priorityDomains) {
    ...
    matchScore += dnaCountHits(joinedThemes, terms)
  }
...
'''.strip()
    if "deriveDnaConfidence(" not in txt:
        txt += "\n\n" + helper + "\n"
    if "const enhancedConfidence = deriveDnaConfidence({" not in txt:
        m = re.search(r'\n(\s*)return\s*{', txt)
        if m:
            indent = m.group(1)
            block = f'''
{indent}const enhancedConfidence = deriveDnaConfidence({{
...
'''
        if inject_after:
            ...
            txt = helper + "\n" + txt
    if "checkExistingDnaReportLock" in txt and "status: 409" not in txt:
        candidates = [
        ...
        for rgx in candidates:
            ...
            if m:
                ...
                block = f'''
const dnaLock = await checkExistingDnaReportLock(supabase, {name})
if (dnaLock.locked) {{
  return Response.json({{
    ok: false,
    locked: true,
    message: "Bu vaka için rapor daha önce oluşturulduğu için yeni rapor üretimi kapatılmıştır.",
    existingReportCreatedAt: dnaLock.existing?.created_at ?? null,
  }}, {{ status: 409 }})
}}
'''
                txt = txt[:m.end()] + "\n" + block + txt[m.end():]
