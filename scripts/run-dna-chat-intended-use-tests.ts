...
async function main() {
  ...
  assert.doesNotMatch(assistantSource, /DNA_INTELLIGENCE_COMPOSER_NOTICE_TR/)
  assert.match(assistantSource, /DNA_INTELLIGENCE_AUDIT_NOTICE_TR/)
  assert.doesNotMatch(assistantSource, /DNA_INTELLIGENCE_PUBLIC_INTENDED_USE\.runtimeTr/)
  ...
  const rootLayoutSource = readWorkspaceFile("src/app/layout.tsx")
  ...
  assert.match(landingHeroSource, /DNA_INTELLIGENCE_LANDING_DESCRIPTION_TR/)
  assert.match(landingHeroSource, /DNA_INTELLIGENCE_LANDING_BOUNDARY_TR/)
  assert.doesNotMatch(landingHeroSource, /deterministik|haricî LLM|model API|internetten bilgi arama/i)
  assert.match(rootLayoutSource, /DNA_INTELLIGENCE_LANDING_DESCRIPTION_TR/)
  assert.doesNotMatch(rootLayoutSource, /DNA_INTELLIGENCE_PLATFORM_DESCRIPTION_TR/)
  assert.match(solutionsSource, /Klinik Planlama Eğitimi/)
  assert.match(solutionsSource, /rapor taslağını inceleyin, düzenleyin, kaydedin ve paylaşın/)
  ...
  assert.match(finalCtaSource, /DNA_INTELLIGENCE_LANDING_DESCRIPTION_TR/)
  assert.match(finalCtaSource, /DNA_INTELLIGENCE_LANDING_BOUNDARY_TR/)
