...
} from "./run-dna-one-shot-locked-evaluation-v3.mjs"
import {
...
function resolveRoot(requested, requireResearchSsd = false) {
  ...
  if (!existsSync(root)) fail("dna_locked_v3_result_root_missing")
  const metadata = lstatSync(root)
  if (metadata.isSymbolicLink() || !metadata.isDirectory()) {
    fail("dna_locked_v3_result_root_invalid")
  }
  const real = realpathSync(root)
  if (real !== root) fail("dna_locked_v3_result_root_realpath_mismatch")
  if (requireResearchSsd && real !== "/Volumes/ResearchSSD"
    && !real.startsWith(`/Volumes/ResearchSSD${sep}`)) {
    fail("dna_locked_v3_result_local_fallback_forbidden")
  }
  return real
}
...
function secureRead(root, relativePath, mode) {
  if (typeof relativePath !== "string" || !relativePath || relativePath.startsWith("/")
    || relativePath.includes("..")) fail("dna_locked_v3_result_path_invalid")
  const path = resolve(root, relativePath)
  const delta = relative(root, path)
  if (!delta || delta === ".." || delta.startsWith(`..${sep}`) || delta.startsWith(sep)) {
    fail("dna_locked_v3_result_path_escape")
  }
  let current = root
  for (const segment of delta.split(sep).filter(Boolean)) {
    current = join(current, segment)
    if (!existsSync(current)) fail("dna_locked_v3_result_file_missing")
    const metadata = lstatSync(current)
    if (metadata.isSymbolicLink()) fail("dna_locked_v3_result_symlink_forbidden")
  }
  const metadata = lstatSync(path)
  if (!metadata.isFile()) fail("dna_locked_v3_result_not_regular_file")
  if (mode !== undefined && (metadata.mode & 0o777) !== mode) {
    fail("dna_locked_v3_result_mode_mismatch")
  }
  const real = realpathSync(path)
  if (real !== path || !real.startsWith(`${root}${sep}`)) {
    fail("dna_locked_v3_result_realpath_escape")
  }
  ...
  try {
    ...
    fail("dna_locked_v3_result_json_invalid")
  }
...
export function buildAggregateManifest(input) {
  ...
  if (result.adapterSha256 !== authority.adapter.adapterSha256
    || result.holdoutSha256 !== authority.holdoutManifest.hashes.sealedFileSha256
    ...
    fail("dna_locked_v3_result_authority_binding_mismatch")
  }
  if (result.counts.total !== EXPECTED_COUNTS.total
    ...
    || result.counts.safetyControls !== 28) {
    fail("dna_locked_v3_result_count_mismatch")
  }
  const gate = qualityGate(result.metrics)
  const manifestBase = {
    schemaVersion: "dna-turkish-retrieval-locked-evaluation-v3-manifest@1",
    recordedAt: result.recordedAt,
    evaluationClass: "internal_blind_source_derived_v3_not_independent_human_validation",
    runId: result.runId,
    authority: {
      ...authority.authority,
  ...
  for (const hash of [input.resultFileSha256, input.claimFileSha256]) {
    if (typeof hash !== "string" || !/^[a-f0-9]{64}$/.test(hash)) {
      fail("dna_locked_v3_result_file_hash_invalid")
    }
  }
...
function assertPublicManifest(manifest) {
  assertExactKeys(manifest, [
    ...
    "manifestSha256",
  ], "dna_locked_v3_public_manifest_fields_invalid")
  if (manifest.schemaVersion !== "dna-turkish-retrieval-locked-evaluation-v3-manifest@1"
    ...
    fail("dna_locked_v3_public_manifest_invalid")
  }
  return manifest
}
...
function verify() {
  ...
  if (stableJson(recorded) !== stableJson(expected)) {
    fail("dna_locked_v3_public_manifest_drift")
  }
  return {
    ok: true,
    written: false,
...
function syntheticMetrics(overrides = {}) {
  const splitAccuracy = [
    ["ambiguous", 42], ["hard_neighbor", 42], ["natural_supported", 56],
    ["safe_theory_control", 28], ["unsupported", 28],
  ].map(([split, count]) => ({ split, count, correct: count, accuracy: 1 }))
  return {
    overallAccuracy: 1,
    splitAccuracy,
    intentAccuracy: Object.entries(EXPECTED_INTENTS).map(([intent, count]) => ({
      intent, count, correct: count, accuracy: 1,
    })),
    topicAccuracy: TOPIC_IDS.map((topicId) => ({
      topicId, count: 9, correct: 9, accuracy: 1,
    })),
...
function syntheticAuthority() {
  const authorityPayload = {
    ...
    workingAdapterFileSha256: "c".repeat(64),
    ...
    freezeManifestFileSha256: "2".repeat(64),
    evaluatorCodeSha256: "4".repeat(64),
    developmentGeneratorFileSha256: "5".repeat(64),
    artifactBuilderFileSha256: "6".repeat(64),
    ...
    holdoutSealedPayloadSha256: "8".repeat(64),
    evaluationCodeSha256: "0".repeat(64),
  }
  return {
    adapter: { adapterSha256: authorityPayload.adapterSha256 },
    holdoutManifest: { hashes: { sealedFileSha256: authorityPayload.holdoutSha256 } },
...
function syntheticResult(authority, metricOverrides = {}) {
  ...
  const payload = {
    schemaVersion: "dna-one-shot-locked-evaluation-v3-result@1",
    label: "internal_locked_holdout_v3_not_independent_human_validation",
    runId: `locked-eval-v3:${"a".repeat(32)}`,
    recordedAt: "2026-07-24T00:00:01.000Z",
    adapterSha256: authority.adapter.adapterSha256,
    holdoutSha256: authority.holdoutManifest.hashes.sealedFileSha256,
    ...
    authoritySha256: authority.authoritySha256,
    counts: {
      total: 196, answerControls: 126, clarificationControls: 42,
      abstentionControls: 28, safetyControls: 28,
    },
    metrics,
...
function syntheticClaim(authority) {
  const payload = {
    schemaVersion: "dna-one-shot-locked-evaluation-v3-claim@1",
...
function expectFailure(action) {
  ...
  if (!failed) fail("dna_locked_v3_verifier_expected_failure_missing")
}
...
function test() {
  ...
  check("aggregate_manifest_success", () => {
    ...
    if (manifest.qualityGate.status !== "pass" || !manifest.boundaries.aggregateOnly) {
      fail("dna_locked_v3_verifier_success_failed")
    }
  })
  ...
  check("quality_failure_preserved", () => {
    ...
    if (manifest.qualityGate.status !== "fail") fail("dna_locked_v3_quality_failure_hidden")
  })
  ...
  check("question_and_failure_id_absence", () => {
    ...
    if (/failureIds|tr-v3-blind-|benzersiz sentetik soru/i.test(serialized)) {
      fail("dna_locked_v3_verifier_private_payload_leak")
    }
  })
  ...
  const temporary = mkdtempSync(join(volumeRoot,
    "Outputs/SelfMetaAI/dna-intelligence/.locked-v3-result-verifier-test-"))
  try {
...
function parseCommand(argv) {
  if (argv.length !== 1 || !["build", "verify", "test"].includes(argv[0])) {
    fail("dna_locked_v3_result_cli_invalid")
  }
  return argv[0]
}
