...
def patch_ai_rewrite():
    path = Path("src/lib/dna/aiRewrite.ts")
    if not path.exists():
        return
    txt = read(str(path))
    ...
    if "DNA_MAX_OUTPUT_TOKENS" not in txt:
        block = '''
...
'''
    ...
    if "max_output_tokens: DNA_MAX_OUTPUT_TOKENS" not in txt and "max_completion_tokens: DNA_MAX_OUTPUT_TOKENS" not in txt:
        txt = re.sub(
            r'(\{\s*model\s*:\s*DNA_MODEL\s*,)',
            r'\1\n      temperature: DNA_TEMPERATURE,\n      max_output_tokens: DNA_MAX_OUTPUT_TOKENS,',
            txt,
            count=1
        )
...
def patch_ai_prompt():
    path = Path("src/lib/dna/aiClinicalPrompt.ts")
    if not path.exists():
        return
    txt = read(str(path))
...
'''.strip()
    ...
    for rgx in [
        ...
        m = re.search(rgx, txt)
        if m:
            if "${DNA_CLINICAL_STYLE_GUIDE}" not in txt[m.start():m.start()+250]:
                txt = txt[:m.end()] + "${DNA_CLINICAL_STYLE_GUIDE}\n\n" + txt[m.end():]
            inserted = True
            break

    if not inserted and "DNA_CLINICAL_STYLE_GUIDE" not in txt:
...
def patch_clinical_analysis():
    path = Path("src/lib/dna/clinicalAnalysis.ts")
    if not path.exists():
        return
    txt = read(str(path))
...
def patch_api_route():
    ...
    if "checkExistingDnaReportLock" not in txt:
        helper = '''
    ...
    if 'status: 409' not in txt:
        for rgx in [
            ...
            if m:
                ...
                block = (
                    "\n"
                    + f"const dnaLock = await checkExistingDnaReportLock(supabase, {var_name})\n"
                    + "if (dnaLock.locked) {\n"
