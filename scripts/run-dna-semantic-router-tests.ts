...
assert.equal(status.questionFrameVersion, "dna-question-frame@2")
...
assert.equal(frame.subquestions[0].normalizedQuestion, "insula nedir")
assert.equal(frame.subquestions[0].topicCandidates[0], "cns.insula")
assert.ok(frame.subquestions[0].topicConfidence >= 0.7)
assert.equal(frame.subquestions[0].relationshipConfidence, 1)
