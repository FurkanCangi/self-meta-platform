...
- `/explicit-consent`: özel nitelikli veri, çocuk/danışan verisi, deterministik rapor işleme ve gerekirse yurt dışı altyapı aktarımı için açık rıza.
...
- Supabase/Vercel ve diğer barındırma sağlayıcıları için yurt dışı aktarım maddesi yeterli mi? Klinik rapor üretiminde harici LLM sağlayıcısı kullanılmadığı açıkça yazılmalı mı?
